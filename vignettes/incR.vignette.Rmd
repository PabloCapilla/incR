...
incubation behaviour using the *incR* R package and present its funtions. This package is
currently available
...
potentically increasing the sensitibity to detect short incubation off-bouts. 

At the heart of *incR* lays *incRscan*; however, *incR* aims at being more than only
such function. If the job of *incRscan* is performed by other means (or *incRscan* is 
modified), *incR* still provides a suite of functions that allow the extraction of biogically
meaningful metrics from a series of observations of the incubating individual. 
This allows for
the development of *incRscan* without affecting the functionallity of the package
as a whole.

## 1.2 - Description of the algorithm implemented by *incRscan*

The idea of developing this approach came up studying blue and great tits in the northerm 
hemisphere in temperate zones. The characteristic nest temperature of incubation
in these species varies around 35ºC, therefore, always well above environmental temperature.
...
temperature 
(the gradient between environmental temperature and incubation temperature ~35ºC is large). 

This dynamic in nest temperature is exploited by *incRscan*, setting thresholdsin temperature
variation which characterised incubation on- and off-bouts. The algorithm is thought to work
well when there is a marked difference between incubation and environmental temperatures. The utility of the algorithm to analyse data collected in hot conditions has not been tested yet.
In this scenario the difference between incubation and
...
be easily extended to understand the analysis of several sequential days. 

I differenciatte two scenrios: i) when nest temperature is higher than
...
temperature between two consecutive data points  (eg. minimum (T~i~ – T~i-1~), this is
...
depending on the species of study). 

"maxIncrease" is defined as the maximum difference in
temperature between two consecutive data points  (eg. maximum (T~i~ – T~i-1~), equal to the
...
incubation on-bouts in scenario ii) is set to 0, as any increase in nest temperature well
above environmental values will indicate the incubating individual has come to the nest.
...
$$T~i~ - T~i-1~ < maxDrop (= minimum (T~i~ – T~i-1~)) (1)$$
$$T~i~ - T~i-1~ > maxIncrease (= maximum (T~i~ – T~i-1~)) (2)$$

T~i~ being the i*th* temperature record from i=2 to i=I (I equal to the last data point, and
total number of
temperature recordings). Off-bout periods are, then, defined between T~i's~ satisfying (1) and
the closest forward situation in which T~j~, when i < j, satisfies (2) (raise in temperature
above 
...
between T~i~ and T~i-1~ indicates the incubating individual has left the nest. To make this
calculation conservative and robust against highly variable calibrating periods, whenever
|*maxDrop*| > *maxNightVar_accepted* (in degrees C) for a particular studied day, the value
of "maxDrop" of the previous day is instead applied. The result of this
...
off-bout, an off-bout comes after a short on-bout (Figure below). To circunvent this problem,
...
*sensitivity* and *temp.diff* work well for blue and great tits in northerm latitudes in 
...
```{r echo = FALSE}
data(incRdataExample2)
df1 <- incRdataExample2 #reading data
str(df1) # preliminar exploration
df.prep <- incRprep (data= df1,                   
                      date.name="DATE",         
                      date.format="%d/%m/%Y %H:%M",
                      timezone="GMT",
                      temperature.name="valueT")
inc.example <- incRscan (data=df.prep,             
                          lower.time=22,           
                          upper.time=3,            
                          sensitivity=0.15,        
                          temp.diff=10,             
...
plot.example <- inc.example[inc.example$date=="2015-05-09",]
# plot
plot (valueT ~ dec.time, data=plot.example,
      pch=19, 
      main=list("Blue tit incubation 07/05/2015"),
      ylim=c(5,38),
      col=c("red", "black")[plot.example$inc.vector+1],
      xaxt="n", las=1, cex.axis=1.5,
      xlab=list("Time", cex=1.5),
      ylab=list("Temperature (ºC)", cex=1.5))
lines (env.temp ~ dec.time, data=plot.example)
axis (side=1,                       # x-axis
...
```
**Figure 1**. Incubation and environmental temperatures. Classification of on- and off-bouts performed in incR.
...
``` 
**Figure 2.** A day of blue tit incubation. Onset and offset estimations are illuestrated by grey-dashed lines.
